...
- 正向：**50/30/10 低余量告警首次真正生效**（此前 `UpdateUi` 从不执行，
  `CheckLowQuotaNotification` 从未触发过——用户从未收到任何余量通知）；首刷成功；
  整个间接层删除，此类 bug 不再可能发生。
- 负向：无。行为变化即修复本身。

## 验收标准（2026-08-12 修订）

修订说明：实施时确认图标为**固定样式**（白色圆角方块 + 蓝色 Q，`IconFactory.Get`
的 color 参数仅用于签名兼容），不存在"图标变色"行为——原"图标即为对应余量颜色"的
表述有误，更正如下：

- 首次刷新后日志无 "no UI context" 记录；首刷数据成功展示。
- 余量跌破 50%/30%/10% 时系统通知**首次可触发**（此前从未触发）。
- tooltip / 详情窗行为不变。
